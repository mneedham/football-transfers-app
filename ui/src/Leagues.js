...
const TOTAL_COUNT_QUERY = gql`
  query topSpendingQuery(
    $country: String
    $orderBy: [_SpendingByLeagueOrdering]
  ) {
    spendingByLeague(countrySubstring: $country, orderBy: $orderBy) {
      league
    }
  }
`;
  ...
  updateTotalRowCount() {
    this.props.client
      ...
      .then(result => {
        ...
        if (data && data.spendingByLeague) {
          this.handleCount(data.spendingByLeague.length);
        }
      });
  }
                ...
                <Table className={this.props.classes.table}>
                  <TableHead>
                    <TableRow>
                      <TableCell
                        key="league"
                        sortDirection={orderBy === "league" ? order : false}
                      >
                        League
                      </TableCell>
                      <TableCell
                  ...
                  <TableBody>
                    {data.spendingByLeague.map(n => {
                      return (
                        <TableRow key={n.league}>
                            ...
                            <div>
                              {n.leagueImage ? (
                                <Avatar
                                  ...
                                  alt={n.league}
                                  src={n.leagueImage.replace("tiny", "medium")}
                                />
                              ) : null}

                              <Link
                                href={"/league-spending/" + n.league}
                                onClick={e => {
                                  e.preventDefault();
                                  navigate("/league-spending/" + n.league);
                                }}
                              >
                                {n.league}
                              </Link>
...
export default withStyles(styles)(withApollo(Leagues));
